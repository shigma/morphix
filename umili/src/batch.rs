...
impl Batch {
    ...
    pub fn load(&mut self, mut change: Change, prefix: &str) -> Result<(), MutationError> {
        ...
        match change {
            ...
            #[cfg(feature = "append")]
            Change::Append { p, v } => match &mut batch.change {
    ...
    pub fn dump(self) -> Option<Change> {
        let mut changes = vec![];
...
mod test {
    ...
    fn batch() {
        ...
        assert_eq!(batch.dump(), Some(Change::set("foo/bar", 1).unwrap()));

        let mut batch = Batch::new();
        batch.load(Change::set("foo/bar", 1).unwrap(), "").unwrap();
        batch.load(Change::set("foo/bar", 2).unwrap(), "").unwrap();
        assert_eq!(batch.dump(), Some(Change::set("foo/bar", 2).unwrap()));

        let mut batch = Batch::new();
        batch
            .load(Change::set("foo/bar", json!({"qux": "1"})).unwrap(), "")
            .unwrap();
        batch.load(Change::append("foo/bar/qux", "2").unwrap(), "").unwrap();
        assert_eq!(
            batch.dump(),
            Some(Change::set("foo/bar", json!({"qux": "12"})).unwrap())
        );
        ...
        batch
            .load(Change::set("foo/bar", json!({"qux": "1"})).unwrap(), "")
            .unwrap();
        assert_eq!(batch.dump(), Some(Change::set("foo/bar", json!({"qux": "1"})).unwrap()));
        ...
        batch
            .load(
                Change::batch(
                    "foo",
                    vec![Change::append("bar", "1").unwrap(), Change::append("bar", "2").unwrap()],
                ),
                "",
            )
            .unwrap();
        assert_eq!(batch.dump(), Some(Change::append("foo/bar", "12").unwrap()));
        ...
        assert_eq!(
            ...
            Some(Change::batch(
                "",
                vec![Change::append("bar", "2").unwrap(), Change::append("qux", "1").unwrap()]
            ))
        );
